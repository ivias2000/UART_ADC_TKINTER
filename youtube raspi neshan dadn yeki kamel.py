...
def connect_menu_init():
    ...
    graph.text = graph.canvas.create_text(
        200, 150, anchor=E, font=("Helvetica", "20"), text="---")
    # Static
    graph.canvas.create_text(
        120, 150, anchor=CENTER, font=("Helvetica", "20"), text="No.")
...
def graph_control(graph):
    ...
    graph.canvas.itemconfig(
        graph.outer, exten=int(graph.sensor))
    graph.canvas.itemconfig(
        graph.text, text=f"{int(graph.sensor)}")
...
def connexion():
    ...
    if connect_btn["text"] in "Disconnect":
        ...
        try:
            ser = serial.Serial(port='/dev/ttyS0',baudrate = baud,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS,timeout=1000)
